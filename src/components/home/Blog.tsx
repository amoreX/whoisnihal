        ...
        return <Link
          ...
          >
            ...
            <div className="z-40 w-full flex sm:flex-row flex-col justify-between size-full items-start sm:items-center">
              <div className="font-medium flex gap-2 sm:whitespace-nowrap">{post.title} <div className='text-foreground/50 hidden sm:flex'>{post.desc.slice(0,30)}...</div></div>
              <div className="text-foreground/80 font-medium shrink-0">{post.date}</div>
            </div>
          </Link>
      })}
    </div>
